...
extern void init_cove(), draw_cove(float x, float y, float z, float yaw,
		float pitch, float roll), draw_decorations();
...
public:
	MyVRApp(const std::string &mySetup) :
		...
		_virtualToRoomSpace = CoordinateFrame(Vector3(1300.0, 0.8, 460.0))
				* _virtualToRoomSpace; //FOR AXIAL in Yurt
	...
	void doUserInput(Array<VRG3D::EventRef> &events) {
		...
		for (int i = 0; i < events.size(); i++) {
			if (events[i]->getName() != "SynchedTime" && events[i]->getName()
					!= "Wand_Tracker" && events[i]->getName() != "Head_Tracker"
					&& events[i]->getName() != "aimon_13"
					&& events[i]->getName() != "aimo_14"
					&& events[i]->getName() != "aimon_15"
					&& events[i]->getName() != "aimon_16") {
			...
			// Save all the tracker events that come in so we can use them in the doGraphics routine
				...
				if (_trackerFrames.containsKey(events[i]->getName())) {
					_trackerFrames[events[i]->getName()]
				...
				Vec3d look_at = g_Draw.getCamera().getLookAt();
				_virtualToRoomSpace = CoordinateFrame(
						Vector3(look_at[0], look_at[1], look_at[2]))
						* _virtualToRoomSpace;
			} else if (events[i]->getName() == "kbd_LEFT_down") {
				_virtualToRoomSpace
						= CoordinateFrame(
								Matrix3::fromAxisAngle(Vector3(0, 1, 0),
										toRadians(0.5))) * _virtualToRoomSpace;
				...
				_virtualToRoomSpace
						= CoordinateFrame(
								Matrix3::fromAxisAngle(Vector3(1, 0, 0),
										toRadians(0.5))) * _virtualToRoomSpace;
				...
				if (get_time_ui_active_index() >= 0) {
					...
					_virtualToRoomSpace = CoordinateFrame(Vector3(0, 0, 0.01))
							* _virtualToRoomSpace;
				}
				...
				if (get_time_ui_active_index() >= 0) {
					...
					_virtualToRoomSpace = CoordinateFrame(Vector3(-0.01, 0, 0))
							* _virtualToRoomSpace;
				}
			...
			} else if (events[i]->getName() == "kbd_U_down") { //move forward
				_virtualToRoomSpace = CoordinateFrame(Vector3(0, 0, 0.01)) * _virtualToRoomSpace;
			} else if (events[i]->getName() == "kbd_J_down") { //move back
				_virtualToRoomSpace = CoordinateFrame(Vector3(0, 0, -0.01)) * _virtualToRoomSpace;
			} else if (events[i]->getName() == "kbd_SHIFT_UP_down") { //move up
				_virtualToRoomSpace = CoordinateFrame(Vector3(0, 0.01, 0)) * _virtualToRoomSpace;
			} else if (events[i]->getName() == "kbd_SHIFT_DOWN_down") { //move down
				_virtualToRoomSpace = CoordinateFrame(Vector3(0, -0.01, 0)) * _virtualToRoomSpace;
				...
				if (prevEvent != "aimon_03" && prevEvent
						!= "Wand_Left_Btn_down") {
					if (get_time_ui_active_index() >= 0) {
						set_time_ui_value(1);
					}
				...
				if (prevEvent != "aimon_04" && prevEvent
						!= "Wand_Right_Btn_down") {
					if (get_time_ui_active_index() >= 0) {
						set_time_ui_value(-1);
					}
			...
			if (fabs(joystick_x) > 0.01) {
				...
				angle /= 20.0;
				CoordinateFrame rotation = CoordinateFrame(
						Matrix3::fromEulerAnglesXYZ(0, angle, 0));
				_virtualToRoomSpace = rotation * _virtualToRoomSpace;
			}

			// Translate#include "timeWidget.h"
			if (fabs(joystick_y) > 0.0 && _trackerFrames.containsKey(
					"Wand_Tracker") == true) {
				if (joystick_y < 0) {
					_virtualToRoomSpace
							= CoordinateFrame(Vector3(0, 0, -0.001))
									* _virtualToRoomSpace;
					//_virtualToRoomSpace.translation = -0.01f * Vector3(0,0,1);
				} else {
					//_virtualToRoomSpace.translation = Vector3(0,0,0);
					_virtualToRoomSpace = CoordinateFrame(Vector3(0, 0, 0.001))
							* _virtualToRoomSpace;
		...
		for (int i = 0; i < trackerNames.size(); i++) {
			...
			if (trackerNames[i] == "Wand_Tracker") {
				...
				glVertex3f(trackerFrame.translation.x,
						trackerFrame.translation.y, trackerFrame.translation.z);
...
int main(int argc, char **argv) {
	...
	g_Set.m_StartupFile = g_Env.m_CurFilePath
			+ "datasvr/worlds/Earthquakes.cml";
